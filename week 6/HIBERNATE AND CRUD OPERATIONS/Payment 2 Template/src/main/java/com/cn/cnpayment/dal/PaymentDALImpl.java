...
public class PaymentDALImpl implements PaymentDAL{
	...
	public void update(Payment updatePayment) {
		// TODO Auto-generated method stub
		Session session = entityManager.unwrap(Session.class);
		Payment currentData = session.get(Payment.class, updatePayment.getId());
		currentData.setDescription(updatePayment.getDescription());
		currentData.setPaymentType(updatePayment.getPaymentType());
	...
	public void updateDescription(int paymentId, String description) {
		// TODO Auto-generated method stub
		Session session = entityManager.unwrap(Session.class);
		Payment currentData = session.get(Payment.class, paymentId);
